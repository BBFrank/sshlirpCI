...
int main() {
    ...
    while (1) {
        if (terminate_daemon_flag) {
            printf("Segnale di terminazione ricevuto prima dell'avvio operazioni, uscita in corso...\n");
            break;
        }
        ...
        if (round > 0) {
            new_commit = check_new_commit(sshlirp_source_dir, sshlirp_repo_url, libslirp_source_dir, libslirp_repo_url, log_file);
        }
        ...
        if (round == 0 || new_commit == 2) {
            ...
            if (round == 0) {
                printf("Primo avvio del demone, è il momento di lanciare i thread...\n");
            } else {
                printf("\n");
                log_time();
                printf("Nuovo commit per sshlirp trovato, procedo con la build...\n");
            }
            ...
            for (int i = 0; i < num_archs; i++) {
                ...
                if (pthread_create(&threads[i], NULL, build_worker, &args[i]) != 0) {
                    ...
                    printf("Thread creato con successo per l'architettura %s.\n", args[i].arch);
                }
            }
            ...
            for (int i = 0; i < num_archs; i++) {
                ...
                if (pthread_join(threads[i], &thread_return_value) != 0) {
                    ...
                    if (thread_return_value != NULL) {
                        int *worker_status = (int *)thread_return_value;
                        printf("Thread per %s terminato con stato: %d\n", args[i].arch, *worker_status);
                        free(worker_status);
                    } else {
                        printf("Thread per %s terminato senza un valore di ritorno specifico (o errore nell'allocazione del ritorno).\n", args[i].arch);
                    }
                }
            }
            ...
            for (int i = 0; i < num_archs; i++) {
                ...
                if (thread_log_read_on_host && thread_log_read_in_chroot) {
                    ...
                    if (thread_log_truncate) {
                        ...
                        printf("Log del thread %s (Host) pulito con successo: %s\n", args[i].arch, thread_log_path_on_host);
                    } else {
                        fprintf(stderr, "Errore nella pulizia (truncating) del log del thread per l'architettura %s: %s. Errore: %s\n", args[i].arch, thread_log_path_on_host, strerror(errno));
                    }
                    ...
                    if (thread_chroot_log_truncate) {
                        ...
                        printf("Log del thread %s (Chroot) pulito con successo: %s\n", args[i].arch, thread_log_path_in_chroot);
                    } else {
                        fprintf(stderr, "Errore nella pulizia (truncating) del log del thread per l'architettura %s (Chroot): %s. Errore: %s\n", args[i].arch, thread_log_path_in_chroot, strerror(errno));
                    }

                } else {
                    fprintf(stderr, "Avviso: Impossibile aprire per la lettura i file di log del thread per l'architettura %s. Errore: %s\n", args[i].arch, strerror(errno));
                }
            }
            ...
            for (int i = 0; i < num_archs; i++) {
                ...
                if (access(source_bin_path, F_OK) == 0) {
                    ...
                    if (rename(source_bin_path, final_target_path) != 0) {
                        ...
                        printf("Binario per l'architettura %s spostato con successo in %s.\n", args[i].arch, final_target_path);
                    }
                } else {
                    fprintf(stderr, "Errore: Binario sorgente %s non trovato per l'architettura %s. Spostamento saltato.\n", source_bin_path, args[i].arch);
                }
            }

            printf("\n");
            log_time();
            printf("Build completata per tutte le architetture.\n");
            ...
            fprintf(stderr, "Errore durante il controllo dei nuovi commit.\n");
            break;

        }
        
        if (terminate_daemon_flag) {
            printf("Segnale di terminazione ricevuto prima dello sleep e a operazioni terminate, uscita in corso...\n");
            break;
        }

        update_daemon_state(DAEMON_STATE_SLEEPING);
        log_time();
        printf("Demone in attesa per %d secondi...\n", poll_interval);
        ...
        while(time_left > 0) {
            ...
            if (terminate_daemon_flag) {
                printf("Sleep interrotto da segnale di terminazione.\n");
                break; 
            }
            if (time_left > 0) {
                ...
                printf("Sleep interrotto, %u secondi rimanenti, continuo ad attendere...\n", time_left);
            }
        }
    ...
    log_time();
    printf("sshlirp_ci demone terminato.\n");
