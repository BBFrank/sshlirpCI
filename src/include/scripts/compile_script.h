...
static const char compile_script_content[] =
    ...
    "fi\n"
    "\n"
    "# Get the absolute path of the chroot log file (I only need it for the first potential log)\n"
    ...
    "\n"
    "# If on trixie, I must first modify CMakeLists.txt to correct the symbol import\n"
    "# of sysprof (add fake.c - if it exists in the sshlirp source directory - in add_executable(sshlirp sshlirp.c libvdeslirp.c libvdestream.c autoopt.c),\n"
    "# which must then become: add_executable(sshlirp sshlirp.c libvdeslirp.c libvdestream.c autoopt.c fake.c)\n"
    "if [ \"$arch\" != \"arm64\" ]; then\n"
    "    echo \"From compile.sh (inside chroot): Modifying CMakeLists.txt for trixie release...\"\n"
    "    # Check if fake.c exists\n"
    "    if [ -f \"fake.c\" ]; then\n"
    ...
    "fi\n"
    "\n"
    "# Verify that the binary was installed correctly and that it is a static executable. Finally, rename it\n"
    "# as sshlirp-<arch>\n"
