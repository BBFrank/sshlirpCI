...
static const char modify_vdens_script_content[] =
    ...
    "exec >> $log_file 2>&1\n"
    "\n"
    "# Check that the vdens source file exists\n"
    "if [ ! -f \"$vdens_c_path\" ]; then\n"
    "    echo \"Error: From modifyVdens.sh: vdens.c file does not exist at $vdens_c_path\"\n"
    "    exit 1\n"
    "fi\n"
    "\n"
    "# Check if the correct modifications have already been applied to vdens.c (I check if the comments I would make are already present)\n"
    "# note: I can't check .git because it seems that after the pull there is no .git dir\n"
    ...
    "fi\n"
    "\n"
    "# Modify the vdens.c file to comment out the calls to uid_gid_map and the CLONE_NEWUSER flag.\n"
