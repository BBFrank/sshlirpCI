...
void *build_worker(void *arg_ptr) {
    ...
    if (args->pull_round == 0) {
        ...
        if (setup_chroot(args, thread_log_fp) != 0) {
            fprintf(thread_log_fp, "Failed to check/setup chroot for %s.", args->arch);
            char err_buf[MAX_CONFIG_ATTR_LEN*2];
        ...
        fprintf(thread_log_fp, "Chroot setup complete for %s.", args->arch);

        // L'operazione di check/creazione delle directory del worker dentro il chroot può avvenire senza lock
        fprintf(thread_log_fp, "Checking and eventually creating worker directories and log file inside chroot for %s.", args->arch);
        if (check_worker_dirs(args, thread_log_fp) != 0) {
            fprintf(thread_log_fp, "Failed to check/create worker directories for %s.", args->arch);
            char err_buf[MAX_CONFIG_ATTR_LEN*2];
        ...
        fprintf(thread_log_fp, "Worker directories and log file checked/created for %s.", args->arch);

    } else {
        fprintf(thread_log_fp, "Not first run (pull_round %d). Skipping chroot setup and dir check for %s.", args->pull_round, args->arch);
    }

    fprintf(thread_log_fp, "Copying sources into chroot for %s.", args->arch);
    ...
    if (copy_sources_to_chroot(args, thread_log_fp) != 0) {
        fprintf(thread_log_fp, "Failed to copy sources for %s.", args->arch);
        char err_buf[MAX_CONFIG_ATTR_LEN*2];
    ...
    fprintf(thread_log_fp, "Sources copied for %s.", args->arch);

    // Compilazione (avviene dentro il chroot quindi i log andranno in args->thread_chroot_log_file)
    fprintf(thread_log_fp, "Starting compilation process in chroot for %s...", args->arch);
    if (compile_and_verify_in_chroot(args, thread_log_fp) != 0) {
        fprintf(thread_log_fp, "...Compilation process failed for %s. Removing sources copy...", args->arch);
        ...
        if (remove_sources_copy_from_chroot(args, thread_log_fp) != 0) {
            fprintf(thread_log_fp, "Error: Failed to remove sources copy for %s.", args->arch);
            char err_buf[MAX_CONFIG_ATTR_LEN*2];
        ...
        fprintf(thread_log_fp, "Sources copy removed after compilation failure for %s.", args->arch);
        char err_buf[MAX_CONFIG_ATTR_LEN*2];
    ...
    fprintf(thread_log_fp, "...Compilation successful for %s.", args->arch);

    fprintf(thread_log_fp, "Removing sources copy for %s.", args->arch);
    ...
    fprintf(thread_log_fp, "Sources copy removed for %s.", args->arch);

    fprintf(thread_log_fp, "Worker finished successfully for arch %s.", args->arch);
